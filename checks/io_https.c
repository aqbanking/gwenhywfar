...
static int check_io_https1() {
  ...
  if (rv) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Could not resolve hostname (%d)\n", rv);
    return 2;
  }
  rv=GWEN_InetAddr_SetPort(addr, 443);
  if (rv) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Could not set port (%d)\n", rv);
    return 2;
  }
  ...
  if (sk==NULL) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Socket type not supported\n");
    return 2;
  }
  ...
  if (io==NULL) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Could not create socket io layer.\n");
    return 2;
  }
  ...
  if (io==NULL) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Could not create TLS io layer.\n");
    return 2;
  }
  ...
  if (io==NULL) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Could not create HTTP io layer.\n");
    return 2;
  }
  ...
  if (rv) {
    fprintf(stderr,
	    "ERROR in check_io_https1: Could not register io layer.\n");
  ...
  for (j=0; j<2; j++) {
    ...
    if (rv) {
      fprintf(stderr,
	      "ERROR in check_io_https1: Could not connect (%d)\n", rv);
      return 2;
    }
    ...
    if (rv<0) {
      fprintf(stderr,
	      "ERROR in check_io_https1: Could not write (%d)\n", rv);
      return 2;
    }
	...
	if (rv==GWEN_ERROR_TIMEOUT || rv==GWEN_ERROR_EOF)
	  break;
	fprintf(stderr,
		"ERROR in check_io_https1: Could not read (%d)\n", rv);
	return 2;
      }
      else {
	buffer[rv]=0;
    ...
    if (rv) {
      fprintf(stderr,
	      "ERROR in check_io_https1: Could not disconnect (%d)\n", rv);
      return 2;
    }
  }
...
int check_io_https() {
  ...
  fprintf(stderr, "IO https checks... ");
  chks++;
  if (check_io_https1())
    errs++;
  ...
  if (chks) {
    if (chks==errs) {
      fprintf(stderr, "IO https checks: all failed.\n");
    }
    else if (errs) {
      fprintf(stderr, "IO https checks: some failed.\n");
    }
    else {
