...
QT3_PrintDialog::QT3_PrintDialog(const char *docTitle,
                             const char *docType,
                             const char *descr,
                             const char *text,
                             QWidget* parent,
                             ...
                             bool modal,
                             WFlags fl)
:QT3_PrintDialogUi(parent, name, modal, fl)
,_docTitle(docTitle)
,_docType(docType)
,_descr(descr)
,_text(text)
...
QT3_PrintDialog::~QT3_PrintDialog(){
}



void QT3_PrintDialog::loadGuiSetup(){
  GWEN_BUFFER *dbuf;
  ...
  rv=GWEN_Gui_ReadDialogPrefs(GWEN_Buffer_GetStart(dbuf), NULL, &dbConfig);
...
void QT3_PrintDialog::saveGuiSetup(){
  GWEN_DB_NODE *db;
  GWEN_BUFFER *dbuf;
  int rv;
  ...
  rv=GWEN_Gui_WriteDialogPrefs(GWEN_Buffer_GetStart(dbuf), db);
  GWEN_Buffer_free(dbuf);
  if (rv<0) {
...
void QT3_PrintDialog::loadPrinterSetup(QPrinter *printer){
  GWEN_DB_NODE *dbConfig=NULL;
  int rv;
  GWEN_BUFFER *dbuf;
  ...
  if (rv<0) {
    ...
    s=GWEN_DB_GetCharValue(dbConfig, "FontFamily", 0, "Arial");
    if (s)
      _fontFamily=QString::fromUtf8(s);
    _fontSize=GWEN_DB_GetIntValue(dbConfig, "FontSize", 0, 11);
    _fontWeight=QFont::Normal;
    s=GWEN_DB_GetCharValue(dbConfig, "FontWeight", 0, "Normal");
    _fontWeight=QFont::Normal;
    if (s) {
    ...
    s=GWEN_DB_GetCharValue(dbConfig, "PageSize", 0, 0);
    if (s) {
    ...
    s=GWEN_DB_GetCharValue(dbConfig, "Orientation", 0, 0);
    if (s) {
    ...
    i=GWEN_DB_GetIntValue(dbConfig, "Resolution", 0, -1);
    if (i!=-1)
      printer->setResolution(i);

    top=(uint)GWEN_DB_GetIntValue(dbConfig, "Top", 0, -1);
    left=(uint)GWEN_DB_GetIntValue(dbConfig, "Left", 0, -1);
    bottom=(uint)GWEN_DB_GetIntValue(dbConfig, "Bottom", 0, -1);
    right=(uint)GWEN_DB_GetIntValue(dbConfig, "Right", 0, -1);
    ...
    s=GWEN_DB_GetCharValue(dbConfig, "ColorMode", 0, 0);
    if (s) {
    ...
    s=GWEN_DB_GetCharValue(dbConfig, "outputFileName", 0, 0);
    if (s)
      printer->setOutputFileName(QString::fromUtf8(s));
    printer->setOutputToFile(GWEN_DB_GetIntValue(dbConfig, "outputToFile", 0, 0));
  }
...
void QT3_PrintDialog::savePrinterSetup(QPrinter *printer) {
  GWEN_DB_NODE *db;
  GWEN_BUFFER *dbuf;
  int rv;
  const char *s;
...
void QT3_PrintDialog::slotSetup(){
...
void QT3_PrintDialog::slotPrint(){
  QPrinter printer(QPrinter::PrinterResolution);
...
void QT3_PrintDialog::slotHelpClicked() {
  // TODO
}



#include "qt3_printdialog.moc"
