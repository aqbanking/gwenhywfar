...
QT5_GuiDialog::QT5_GuiDialog(QT5_Gui *gui, GWEN_DIALOG *dlg)
  :CppDialog(dlg)
  ,_gui(gui)
  ,_mainWidget(NULL) {
...
QT5_GuiDialog *QT5_GuiDialog::getDialog(GWEN_DIALOG *dlg) {
  CppDialog *cppDlg;

  cppDlg=CppDialog::getDialog(dlg);
  if (cppDlg)
    return dynamic_cast<QT5_GuiDialog*>(cppDlg);
  return NULL;
}



int QT5_GuiDialog::execute() {
  QT5_DialogBox *dialogBox;
  int rv;

  dialogBox=dynamic_cast<QT5_DialogBox*>(getMainWindow());
  if (dialogBox==NULL) {
...
int QT5_GuiDialog::runDialog(bool untilEnd) {
  QT5_DialogBox *dialogBox;
...
int QT5_GuiDialog::setIntProperty(GWEN_WIDGET *w,
...
int QT5_GuiDialog::getIntProperty(GWEN_WIDGET *w,
                                  GWEN_DIALOG_PROPERTY prop,
                                  int index,
                                  int defaultValue) {
...
int QT5_GuiDialog::setCharProperty(GWEN_WIDGET *w,
                                   GWEN_DIALOG_PROPERTY prop,
                                   int index,
                                   const char *value,
                                   int doSignal) {
...
const char *QT5_GuiDialog::getCharProperty(GWEN_WIDGET *w,
    GWEN_DIALOG_PROPERTY prop,
    int index,
...
int QT5_GuiDialog::setupTree(GWEN_WIDGET *w) {
  int rv;
  Qt5_W_Widget *xw=NULL;

  switch(GWEN_Widget_GetType(w)) {
  case GWEN_Widget_TypeDialog:
    xw=new Qt5_W_Dialog(w);
    break;
  ...
  if (rv<0) {
    ...
    if (GWEN_Widget_GetType(w)==GWEN_Widget_TypeDialog)
      _mainWidget=(QT5_DialogBox*) GWEN_Widget_GetImplData(xw->getCInterface(), QT5_DIALOG_WIDGET_REAL);
...
bool QT5_GuiDialog::setup(GWEN_UNUSED QWidget *parentWindow) {
  ...
  _mainWidget=(QT5_DialogBox*) GWEN_Widget_GetImplData(w, QT5_DIALOG_WIDGET_REAL);
