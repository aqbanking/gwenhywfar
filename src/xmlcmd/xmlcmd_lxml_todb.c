...
GWEN_XMLCOMMANDER *GWEN_XmlCommanderLibXml_toDb_new(xmlNodePtr xmlNodeDocument, GWEN_DB_NODE *dbDestination)
{
  ...
  cmd=GWEN_XmlCommanderLibXml_new(xmlNodeDocument, dbDestination);
...
int _handleXmlEnter(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode)
{
  ...
  n=GWEN_XmlCommanderLibXml_GetXmlNode(GWEN_XmlCommanderLibXml_GetCurrentDocNode(cmd), path, GWEN_PATH_FLAGS_PATHMUSTEXIST);
  if (n==NULL) {
  ...
  GWEN_XmlCommanderLibXml_EnterDocNode(cmd, n);
  ...
  GWEN_XmlCommanderLibXml_LeaveDocNode(cmd);
  return 0;
}
...
int _handleXmlForEvery(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode)
{
  ...
  while (n) {
    ...
    n=GWEN_XmlCommanderLibXml_FindNextElement(n, path);
  }
...
int _handleDbCreateAndEnterGroup(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode)
{
  ...
  /* push group */
  dbLast=GWEN_XmlCommanderLibXml_GetCurrentDbGroup(cmd);

  /* create group */
  GWEN_XmlCommanderLibXml_SetCurrentDbGroup(cmd, GWEN_DB_GetGroup(dbLast, GWEN_PATH_FLAGS_CREATE_GROUP, name));

  /* handle children (nothing special here) */
  rv=GWEN_XmlCommander_HandleChildren(cmd, xmlNode);

  /* pop group */
  GWEN_XmlCommanderLibXml_SetCurrentDbGroup(cmd, dbLast);
...
int _handleDbCreateAndEnterTempGroup(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode)
{
  ...
  /* push group */
  dbLast=GWEN_XmlCommanderLibXml_GetCurrentTempDbGroup(cmd);

  /* create group */
  dbCurrent=GWEN_DB_GetGroup(dbLast, GWEN_PATH_FLAGS_CREATE_GROUP, name);
  GWEN_XmlCommanderLibXml_SetCurrentTempDbGroup(cmd, dbCurrent);
  ...
  /* pop group */
  GWEN_XmlCommanderLibXml_SetCurrentTempDbGroup(cmd, dbLast);
...
int _handleDbSetCharValue_internal(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode, GWEN_DB_NODE *dbCurrent)
{
  const char *name;
  const char *value;
  ...
  if (value) {
    ...
    rv=GWEN_DB_ReplaceVars(GWEN_XmlCommanderLibXml_GetCurrentTempDbGroup(cmd), value, dbuf);
    if (rv<0) {
      DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
    ...
    value=GWEN_XmlCommanderLibXml_GetXmlCharValueByPath(GWEN_XmlCommanderLibXml_GetCurrentDocNode(cmd), path, NULL);
    if (value && *value) {
...
{
  return _handleDbSetCharValue_internal(cmd, xmlNode, GWEN_XmlCommanderLibXml_GetCurrentDbGroup(cmd));
}
...
{
  return _handleDbSetCharValue_internal(cmd, xmlNode, GWEN_XmlCommanderLibXml_GetCurrentTempDbGroup(cmd));
}
...
int _handleXmlIfPathExists(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode)
{
  ...
  if (GWEN_XmlCommanderLibXml_GetXmlNode(GWEN_XmlCommanderLibXml_GetCurrentDocNode(cmd), path, GWEN_PATH_FLAGS_PATHMUSTEXIST)) {
    int rv;
...
int _handleXmlIfNotPathExists(GWEN_XMLCOMMANDER *cmd, GWEN_XMLNODE *xmlNode)
{
  ...
  if (NULL==GWEN_XmlCommanderLibXml_GetXmlNode(GWEN_XmlCommanderLibXml_GetCurrentDocNode(cmd), path, GWEN_PATH_FLAGS_PATHMUSTEXIST)) {
    int rv;
