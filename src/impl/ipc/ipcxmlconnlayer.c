...
GWEN_ERRORCODE GWEN_IPCXMLConnLayer_Accept(GWEN_IPCCONNLAYER *cl,
  ...
  /* copy important data */
  GWEN_ConnectionLayer_SetFlags(newcl, GWEN_ConnectionLayer_GetFlags(cl));
...
GWEN_ERRORCODE GWEN_IPCXMLConnLayer_Flush(GWEN_IPCCONNLAYER *cl) {
  ...
  if (ccd->currentMsg) {
    if (GWEN_HBCIMsg_GetNodes(ccd->currentMsg)) {
      ...
      if (GWEN_HBCIMsg_EncodeMsg(ccd->currentMsg)) {
        DBG_WARN(0, "Error encoding message");
        return GWEN_Error_new(0,
                              GWEN_ERROR_SEVERITY_ERR,
                              GWEN_Error_FindType(GWEN_IPC_ERROR_TYPE),
...
void GWEN_IPCXMLConnLayer_SetLocalName(GWEN_IPCCONNLAYER *cl,
  ...
  GWEN_HBCIDialog_SetLocalName(ccd->dialog, n);
}
...
GWEN_ERRORCODE GWEN_IPCXMLConnLayer_SetSecurityFlags(GWEN_IPCCONNLAYER *cl,
  ...
  DBG_INFO(0, "Setting security flags to %08x", flags);
  ccd->msgFlags=flags;
  return 0;
}
...
GWEN_IPCXMLConnLayer_GetCryptKey(GWEN_IPCCONNLAYER *cl){
  ...
  assert(ccd->securityManager);
  return GWEN_IPCXMLSecCtxMgr_GetLocalCryptKey(ccd->securityManager);
}
...
void GWEN_IPCXMLConnLayer_SetCryptKey(GWEN_IPCCONNLAYER *cl,
  ...
  assert(ccd->securityManager);
  GWEN_IPCXMLSecCtxMgr_SetLocalCryptKey(ccd->securityManager, key);
}
