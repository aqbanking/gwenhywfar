...
/* Update the given eData according to the given eDefinitions element if it matches */
int GWEN_Parser__UpdateElement(const GWEN_PARSER_ELEMENT *eDefinitions, GWEN_PARSER_ELEMENT *eData) {
  ...
  s=GWEN_ParserElement_GetDbName(eDefinitions);
  GWEN_ParserElement_SetDbName(eData, s);
...
const GWEN_PARSER_ELEMENT *GWEN_Parser__GetAndUpdateChoice(const GWEN_PARSER_ELEMENT *eDefinitions, GWEN_PARSER_ELEMENT *eData) {
  GWEN_PARSER_ELEMENT *e;

  e=GWEN_ParserElement_Tree_GetFirstChild(eDefinitions);
  while(e) {
    if (0==GWEN_Parser__UpdateElement(e, eData))
      return e;
...
int GWEN_Parser__UpdateElementAndChildren(const GWEN_PARSER_ELEMENT *eDefinitions, GWEN_PARSER_ELEMENT *eData, int depth) {
  ...
  if (GWEN_ParserElement_GetElementType(eDefinitions)==GWEN_ParserElementType_Choice) {
    ...
    /* compare directly */
    rv=GWEN_Parser__UpdateElement(eDefinitions, eData);
    if (rv<0) {
  ...
  rv=GWEN_Parser__UpdateSequence(eDefChild, eDataChild, depth+1);
  if (rv<0) {
    DBG_DEBUG(GWEN_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
...
int GWEN_Parser__UpdateSequence(const GWEN_PARSER_ELEMENT *eDefinitions, GWEN_PARSER_ELEMENT *eData, int depth) {
  ...
  while(d) {
    ...
    rv=GWEN_Parser__UpdateElementAndChildren(d, e, depth+1);
    if (rv==0) {
...
int GWEN_Parser_UpdateTree(const GWEN_PARSER_ELEMENT_TREE *tDefinitions, GWEN_PARSER_ELEMENT_TREE *tData) {
  ...
  rv=GWEN_Parser__UpdateSequence(d, e, 0);
  if (rv<0) {
    DBG_DEBUG(GWEN_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
