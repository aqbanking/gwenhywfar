...
GWEN_INHERIT(GWEN_CRYPT_KEY, GWEN_CRYPT_KEY_RSA2)
...
int GWEN_Crypt_KeyRsa2__getNamedElement(gcry_sexp_t pkey, const char *name, gcry_mpi_t *pMpi) {
  gcry_sexp_t list;
  gcry_mpi_t mpi;
...
int GWEN_Crypt_KeyRsa2_GeneratePair2(unsigned int nbits, int use65537e,
  ...
  if (!pkey) {
    ...
    /* get public exponent */
    rv=GWEN_Crypt_KeyRsa2__getNamedElement(pkey, "n", &n);
    if (rv<0) {
    ...
    rv=GWEN_Crypt_KeyRsa2__getNamedElement(pkey, "e", &e);
    if (rv<0) {
      DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
      gcry_mpi_release(n);
      gcry_sexp_release(key);
    ...
    GWEN_INHERIT_SETDATA(GWEN_CRYPT_KEY, GWEN_CRYPT_KEY_RSA2, k, xk, GWEN_Crypt_KeyRsa2_freeData);
    GWEN_Crypt_Key_SetSignFn(k, GWEN_Crypt_KeyRsa2_Sign);
    GWEN_Crypt_Key_SetVerifyFn(k, GWEN_Crypt_KeyRsa2_Verify);
    GWEN_Crypt_Key_SetEncipherFn(k, GWEN_Crypt_KeyRsa2_Encipher);
    GWEN_Crypt_Key_SetDecipherFn(k, GWEN_Crypt_KeyRsa2_Decipher);
  ...
  if (!pkey) {
    ...
    rv=GWEN_Crypt_KeyRsa2__getNamedElement(pkey, "d", &d);
    if (rv<0) {
      DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
      gcry_mpi_release(e);
      gcry_mpi_release(n);
...
int GWEN_Crypt_KeyRsa2_GeneratePair(unsigned int nbytes, int use65537e,
				   GWEN_CRYPT_KEY **pPubKey,
				   GWEN_CRYPT_KEY **pSecretKey) {
  return GWEN_Crypt_KeyRsa2_GeneratePair2(nbytes*8, use65537e, pPubKey, pSecretKey);
}




int GWEN_Crypt_KeyRsa2_Sign(GWEN_CRYPT_KEY *k,
...
int GWEN_Crypt_KeyRsa2_Verify(GWEN_CRYPT_KEY *k,
			     const uint8_t *pInData,
			     uint32_t inLen,
...
int GWEN_Crypt_KeyRsa2_Encipher(GWEN_CRYPT_KEY *k,
			       const uint8_t *pInData,
			       uint32_t inLen,
			       uint8_t *pOutData,
			       uint32_t *pOutLen) {
...
int GWEN_Crypt_KeyRsa2_Decipher(GWEN_CRYPT_KEY *k,
			       const uint8_t *pInData,
			       uint32_t inLen,
			       uint8_t *pOutData,
			       uint32_t *pOutLen) {
...
int GWEN_Crypt_KeyRsa2__ReadMpi(GWEN_DB_NODE *db, const char *dbName, gcry_mpi_t *pMpi) {
  gcry_error_t err;
  const void *p;
  unsigned int len;
  gcry_mpi_t mpi=NULL;
...
int GWEN_Crypt_KeyRsa2__WriteMpi(GWEN_DB_NODE *db, const char *dbName, const gcry_mpi_t mpi) {
  gcry_error_t err;
  unsigned char *buf;
  size_t nbytes;
...
int GWEN_Crypt_KeyRsa2__MpiToBuffer(const gcry_mpi_t mpi, unsigned char *buf, size_t nbytes) {
  gcry_error_t err;
  size_t nwritten=0;
...
GWENHYWFAR_CB
void GWEN_Crypt_KeyRsa2_freeData(GWEN_UNUSED void *bp, void *p) {
  GWEN_CRYPT_KEY_RSA2 *xk;

  xk=(GWEN_CRYPT_KEY_RSA2*) p;
  if (xk->modulus)
...
GWEN_CRYPT_KEY *GWEN_Crypt_KeyRsa2_fromDb(GWEN_DB_NODE *db) {
  ...
  /* extend key */
  GWEN_NEW_OBJECT(GWEN_CRYPT_KEY_RSA2, xk);
  GWEN_INHERIT_SETDATA(GWEN_CRYPT_KEY, GWEN_CRYPT_KEY_RSA2, k, xk,
		       GWEN_Crypt_KeyRsa2_freeData);
  GWEN_Crypt_Key_SetSignFn(k, GWEN_Crypt_KeyRsa2_Sign);
  GWEN_Crypt_Key_SetVerifyFn(k, GWEN_Crypt_KeyRsa2_Verify);
  GWEN_Crypt_Key_SetEncipherFn(k, GWEN_Crypt_KeyRsa2_Encipher);
  GWEN_Crypt_Key_SetDecipherFn(k, GWEN_Crypt_KeyRsa2_Decipher);
  ...
  /* read data */
  rv=GWEN_Crypt_KeyRsa2__ReadMpi(dbR, "n", &(xk->modulus));
  if (rv<0) {
    DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
    GWEN_Crypt_Key_free(k);
    return NULL;
  ...
  rv=GWEN_Crypt_KeyRsa2__ReadMpi(dbR, "e", &(xk->pubExponent));
  if (rv<0) {
    DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
    GWEN_Crypt_Key_free(k);
    return NULL;
  ...
  if (!isPublic) {
    rv=GWEN_Crypt_KeyRsa2__ReadMpi(dbR, "d", &(xk->privExponent));
    if (rv<0) {
      DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
      GWEN_Crypt_Key_free(k);
      return NULL;
...
int GWEN_Crypt_KeyRsa2_toDb(const GWEN_CRYPT_KEY *k, GWEN_DB_NODE *db, int pub) {
  GWEN_CRYPT_KEY_RSA2 *xk;
  GWEN_DB_NODE *dbR;
  int rv;
  gcry_ac_data_t ds;
  ...
  /* store n */
  rv=GWEN_Crypt_KeyRsa2__WriteMpi(dbR, "n", xk->modulus);
  if (rv) {
    DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
  ...
  /* store e */
  rv=GWEN_Crypt_KeyRsa2__WriteMpi(dbR, "e", xk->pubExponent);
  if (rv) {
    DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
    return rv;
  }
  ...
  if (!pub) {
    /* store d */
    rv=GWEN_Crypt_KeyRsa2__WriteMpi(dbR, "d", xk->privExponent);
    if (rv) {
      DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
      return rv;
    }
...
int GWEN_Crypt_KeyRsa2_GetModulus(const GWEN_CRYPT_KEY *k, uint8_t *buffer, uint32_t *pBufLen) {
  GWEN_CRYPT_KEY_RSA2 *xk;
  int rv;
...
int GWEN_Crypt_KeyRsa2_GetExponent(const GWEN_CRYPT_KEY *k, uint8_t *buffer, uint32_t *pBufLen) {
  GWEN_CRYPT_KEY_RSA2 *xk;
  int rv;
...
int GWEN_Crypt_KeyRsa2_GetSecretExponent(const GWEN_CRYPT_KEY *k, uint8_t *buffer, uint32_t *pBufLen) {
  GWEN_CRYPT_KEY_RSA2 *xk;
  int rv;
...
GWEN_CRYPT_KEY *GWEN_Crypt_KeyRsa2_fromModExp(unsigned int nbytes,
  ...
  /* create key from DB */
  key=GWEN_Crypt_KeyRsa2_fromDb(dbKey);
  if (key==NULL) {
...
GWEN_CRYPT_KEY *GWEN_Crypt_KeyRsa2_fromModPrivExp(unsigned int nbytes,
						 const uint8_t *pModulus,
						 uint32_t lModulus,
						 const uint8_t *pExponent,
						 uint32_t lExponent,
...
GWEN_CRYPT_KEY *GWEN_Crypt_KeyRsa2_dup(const GWEN_CRYPT_KEY *k) {
  GWEN_CRYPT_KEY_RSA2 *xk;
  GWEN_DB_NODE *dbKey;
  GWEN_CRYPT_KEY *nk;
  int rv;
  ...
  rv=GWEN_Crypt_KeyRsa2_toDb(k, dbKey, xk->pub);
  if (rv<0) {
    DBG_INFO(GWEN_LOGDOMAIN, "here (%d)", rv);
    GWEN_DB_Group_free(dbKey);
    return NULL;
  ...
  nk=GWEN_Crypt_KeyRsa2_fromDb(dbKey);
  GWEN_DB_Group_free(dbKey);
  if (nk==NULL) {
    DBG_INFO(GWEN_LOGDOMAIN, "Could not create key");
  }

  GWEN_Crypt_KeyRsa2_SetFlags(nk, xk->flags);
...
uint32_t GWEN_Crypt_KeyRsa2_GetFlags(const GWEN_CRYPT_KEY *k) {
  GWEN_CRYPT_KEY_RSA2 *xk;
...
void GWEN_Crypt_KeyRsa2_SetFlags(GWEN_CRYPT_KEY *k, uint32_t fl) {
  GWEN_CRYPT_KEY_RSA2 *xk;
...
void GWEN_Crypt_KeyRsa2_AddFlags(GWEN_CRYPT_KEY *k, uint32_t fl) {
  GWEN_CRYPT_KEY_RSA2 *xk;
...
void GWEN_Crypt_KeyRsa2_SubFlags(GWEN_CRYPT_KEY *k, uint32_t fl) {
  GWEN_CRYPT_KEY_RSA2 *xk;
