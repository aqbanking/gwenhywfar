...
#include "gwenbuild/builders/tmplib_p.h"
...
GWEN_INHERIT(GWB_BUILDER, GWB_BUILDER_TMPLIB);
...
GWB_BUILDER *GWEN_TmpLibBuilder_new(GWENBUILD *gwenbuild, GWB_CONTEXT *context)
{
  GWB_BUILDER *builder;
  GWB_BUILDER_TMPLIB *xbuilder;
  int rv;

  builder=GWB_Builder_new(gwenbuild, context, "TmpLibBuilder");
  GWEN_NEW_OBJECT(GWB_BUILDER_TMPLIB, xbuilder);
  GWEN_INHERIT_SETDATA(GWB_BUILDER, GWB_BUILDER_TMPLIB, builder, xbuilder, _freeData);
...
void _setArchiverName(GWB_BUILDER *builder, const char *s)
{
  ...
  xbuilder=GWEN_INHERIT_GETDATA(GWB_BUILDER, GWB_BUILDER_TMPLIB, builder);
  free(xbuilder->archiverName);
  ...
  if (s && *s) {
    ...
    if (ext) {
      ...
      if (strcasecmp(ext, "o")==0) {
        DBG_INFO(NULL, "File \"%s\" is acceptable as input for TmpLibBuilder", s);
        return 1;
      }
    }
    DBG_INFO(NULL, "File \"%s\" is NOT acceptable as input for TmpLibBuilder", s);
  }
...
GWB_BUILD_CMD *_genCmd(GWB_BUILDER *builder, GWB_BUILD_CONTEXT *bctx, GWB_FILE_LIST2 *inFileList, GWB_FILE *outFile)
{
  ...
  GWEN_Buffer_AppendString(argBuffer, " Trc "); /* create a flattened thin archive (see "man ar") */
